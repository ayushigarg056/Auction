...
PreparedStatement ps = cn.prepareStatement("select * from electronics where bid_end_time >NOW()");
...
for(int i=0;i<c;){
		...
		k=k+1;
            ...
            <img id=<%="myImg"+(i+j+1)%> src=<%="electronics/"+rs.getInt(1)+".jpg"%> alt="hiii" width="300" height="200"></td><%
        	rs.next();
			}
			%></tr><%
			for(int j=0;j<4&&j<(c-i);j++)
